...
if file_ren and file_real:
    ...
    # --- 5. LAPORAN POIN 5 (REKAP SATKER) ---
    st.divider()
    st.subheader("📑 Laporan Audit Rekonsiliasi (Struktur Poin 5)")
    ...
    for i, s in enumerate(sorted(df_ren[satker_col].dropna().unique()), 1):
        ...
        # Poin 3: Identifikasi Overbudget
        over = merge_s[(merge_s['_merge'] == 'both') & (merge_s[val_col + '_y'] > merge_s[val_col + '_x'])]
        ...
        rekap_list.append({
            ...
            'Penyedia (Sesuai RUP)': merge_s[merge_s['_merge'] == 'both'][val_col + '_y'].sum(),
            'Penyedia (Tidak Sesuai)': merge_s[merge_s['_merge'] == 'right_only'][val_col + '_y'].sum(),
            'Penyedia (Tokodaring)': real_s[real_s['Kat_Audit'] == 'Tokodaring'][val_col].sum(),
            'Selisih Anggaran': ren_s[val_col].sum() - real_s[val_col].sum(),
            'Identifikasi': f"Over: {len(over)} pkt" if len(over) > 0 else "Normal"
        })
    ...
    st.download_button("📥 Download Excel Laporan", buffer.getvalue(), "Audit_PBJ.xlsx")
